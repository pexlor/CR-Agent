...
def checkpoint(task_id: str, sequence: int, predecessor_id: str | None = None) -> Checkpoint:
    return Checkpoint(
...
def test_acquire_renew_and_fence_execution_lease() -> None:
    ...
    with pytest.raises(ValueError):
        service.release_execution(task.task_id, permit.lease_id, permit.fencing_token - 1)


def test_phase_completion_requires_expected_version_lease_and_checkpoint_chain() -> None:
...
def test_expired_lease_is_recoverable_but_clock_queries_do_not_mutate_task() -> None:
    ...
    clock.current = now + timedelta(seconds=121)  # type: ignore[misc]
