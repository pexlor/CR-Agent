...
| 模型协议转换与传输 | 项目自有 Provider adapters + 受限 HTTPX transport | OpenAI、Anthropic、OpenAI-compatible 的确定性 wire request 与响应归一化 |
...
核心层只依赖项目定义的协议和领域模型，不依赖 Typer、PyGithub、python-gitlab、HTTPX 或供应商 SDK 的具体对象。第三方对象必须在适配器边界转换成内部类型。
...
code-review-agent cleanup TASK_ID
```
...
### 6.1 Provider 与 wire transport 范围

采用项目自有的 OpenAI、Anthropic 和 OpenAI-compatible Provider adapter。每个 adapter 实现 `prepare_request()/send_prepared()`：前者一次性构造固定 method、origin、path、headers allowlist 和规范 body 字节；后者通过项目受限 HTTPX transport 原样发送这些字节，除绑定固定 origin 的认证头外不得重新序列化或补充请求。响应由 adapter 归一化为项目内部模型。

不采用 LangChain ChatModel、供应商 SDK 的高层 chat 调用、LangChain Agent、LangGraph、LangSmith 必需后端、自动工具循环、fallback、streaming 或隐式重试。原因是高层调用通常在发送阶段重新序列化，无法证明安全扫描、token 预留、Trace 摘要和真实线上 body 是同一不可变字节序列。预算、重试、安全、trace、结构化校验和恢复由项目自己的 `ModelGateway` 管理。
...
- `running`：外部请求已经开始；
- `succeeded`：Provider 已确认成功并取得响应；有可靠 usage 时按实际值结算，usage 缺失或不可信时保持成功事实，并用独立 `usage_state = missing / untrusted` 表达；
- `failed_known`：明确失败，记录已知 usage 或保留预留值；
- `unknown`：请求可能已被供应商处理，但本地未取得确定结果。

调用状态与预算账户状态正交。可信实际 usage 超过预留时，调用仍按真实结果进入 `succeeded` 或 `failed_known`，预算账户另行进入 `frozen_overage`；不得增加“预算异常”调用终态来混淆两个维度。

Provider adapter 与 HTTP transport 的自动重试均关闭。MVP 不使用流式输出。`unknown` 不自动重试，按预留最大 token 计入不确定消耗并暂停，直到用户显式执行 `resume`。
...
成功调用有可靠 usage 时按实际值结算。Provider 已确认成功但 usage 缺失或不可信时，调用保持 `succeeded`，预留完整转入不确定消耗并记录独立 `usage_state`；只有请求是否送达或 Provider 是否处理结果确实无法确认时，调用才进入 `unknown`。连接中断及结果不明调用同样按完整预留计入不确定消耗。失败调用和显式重试均产生独立预算流水，不覆盖历史消耗。

可信实际 usage 超过预留时，如实结算实际值并单列超出量，冻结后续模型调用，任务暂停并形成部分结果。只有本机可信用户追加授权使其覆盖当前已知实际、不确定消耗和活动预留、显式恢复，并在不改变固定 Provider、模型和 origin 的前提下重新通过硬预算能力验证，才能解除冻结。追加授权本身不自动解冻；下一请求完成最终序列化后仍须独立通过调用前预留门禁。
...
Review Tool 不使用 Python packaging entry point，也不加载扩展 Python 实现。工具以应用内置或随发布物锁定的 TOML/JSON 声明式规则包注册，由系统自有受限解释器执行；绝不从受审仓库、工作目录或网络动态加载。
...
注册器只允许 MVP 能力 `read_authorized_text` 和 `deterministic_analysis`。规则语言仅提供固定的字面量、Token、行谓词和有限窗口操作，禁止通用正则、任意表达式、模板、动态导入及回调。运行时通过输入、Token、规则、步骤、匹配和输出硬上限保证终止；工具不能访问网络、凭证、SQLite、环境变量、进程或通用文件系统。

首个验收工具为静态危险模式检查器：对传入的已授权文本和变更行执行项目内置规则，不调用 shell，不加载仓库配置。增加第二个工具时只新增规则包数据和注册清单，主编排代码保持不变，以此验收 FR-017。
...
暂不采用。其统一路由、fallback、重试和成本能力与项目自有预算及恢复机制重叠。当前只需 OpenAI、Anthropic 和标准 OpenAI-compatible，由项目自有 Provider adapters 与受限 transport 提供足够且可验证的协议能力。

### LangChain ChatModel 与供应商高层 SDK

暂不采用其高层发送接口。虽然能降低协议适配代码量，但发送阶段可能重新序列化或注入默认字段，破坏最终请求字节、安全证明和预算计数的一致性。供应商协议差异由自有 Provider adapter 隔离；未来只有在 SDK 提供公开、稳定且可验证的 wire-request 构造与原样发送接口时才重新评估。
...
最终路线是：**Python 3.12 单机 CLI + 自有模型 Provider adapters/受限 HTTPX transport + 自有显式状态机 + SQLite + SDK 封装的平台 Provider + 统一安全边界**。
